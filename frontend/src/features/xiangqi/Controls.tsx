...
export function Controls({ api }: { api: XiangqiApi }) {
  const {
    mode, setMode, humanColor, setHumanColor, levelId, setLevel,
    newGame, undo, requestHint, flipBoard, aiThinking, moveList, interactive,
  } = api

  const canUndo = moveList.length > 0 && !aiThinking
  const canHint = interactive

...
/**
 * Tailwind arbitrary-variant prose styling for rendered markdown, scoped to the
 * dark design tokens. Reused by the editor preview and the share page so both
 * render identically.
 */
export const proseClass = cn(
  'max-w-none text-[0.95rem] leading-relaxed text-fg-soft',
  '[&_h1]:mt-0 [&_h1]:mb-4 [&_h1]:text-2xl [&_h1]:font-bold [&_h1]:text-fg',
  '[&_h2]:mt-7 [&_h2]:mb-3 [&_h2]:border-b [&_h2]:border-border-soft [&_h2]:pb-2 [&_h2]:text-xl [&_h2]:font-semibold [&_h2]:text-fg',
  '[&_h3]:mt-5 [&_h3]:mb-2 [&_h3]:text-lg [&_h3]:font-semibold [&_h3]:text-fg',
  '[&_h4]:mt-4 [&_h4]:mb-2 [&_h4]:font-semibold [&_h4]:text-fg',
  '[&_p]:my-3',
  ...
  '[&_pre]:my-4 [&_pre]:overflow-x-auto [&_pre]:rounded-xl [&_pre]:border [&_pre]:border-border-soft [&_pre]:bg-surface-2 [&_pre]:p-4 [&_pre]:text-sm [&_pre]:leading-relaxed',
...
export const MarkdownPreview = forwardRef<HTMLDivElement, { source: string; className?: string }>(
  function MarkdownPreview({ source, className }, ref) {
    return (
      <div ref={ref} className={cn(proseClass, className)}>
